...
current leaf selects the root-to-leaf branch used for context. `/tree`
...
or rewrite output. Do not confuse the extension event `tool_result` with
`toolResult` messages. `tool_result` is an extension hook that can patch the
result before it is committed (`content`, `details`, and/or `isError`; omitted
fields keep their current values). A `toolResult` message is the durable
transcript entry (`role: "toolResult"`) that appears later as
`message_start`/`message_end` and in the session file.
...
For a custom tool, derive the TypeBox input type with `Static` from the
`typebox` package (a direct dependency of coding-agent; not re-exported from
`@earendil-works/pi-coding-agent`) and supply explicit generic parameters to the
guard:
