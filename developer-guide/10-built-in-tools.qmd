...
`read`, `write`, `edit`, `grep`, `find`, and `ls` use the same boundary. Apply
the boundary to `user_bash` as well. A `tool_call` hook can add policy checks,
but operating-system or container controls must enforce isolation.
...
interfaces make each tool's I/O replaceable:
...
BashOperations    exec(command, cwd, env, signal, timeout, onData)
Grep/Find/Ls      operation interfaces for filesystem/search behavior
```

Each line names the boundary an extension replaces: file tools receive focused
filesystem methods, while `bash` receives an execution method with streaming
and cancellation inputs.

An extension can create a tool with remote operations, register it under the
same name, and preserve Pi's validation and rendering behavior. The extension
...
flag, prior component, execution time, and an `invalidate()` callback. Those
inputs support syntax-highlighted read/write previews, compact default
