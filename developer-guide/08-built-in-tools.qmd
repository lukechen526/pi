...
and bounded repository discovery. Each built-in is a tool definition with a
TypeBox parameter schema that Pi wraps as an `AgentTool`, placing built-ins and
extension-provided tools in the same lifecycle.
...
or `pi.setActiveTools()` can change what the model receives.
...
User shell commands and assistant `bash` calls follow distinct paths. The user
directly requests a `!` command, while the model generates a tool call that Pi
records in the assistant/tool-result protocol. A sandbox extension should cover
