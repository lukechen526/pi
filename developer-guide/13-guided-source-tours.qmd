...
dispatcher early, and calls `main(process.argv.slice(2))`. Then follow
...
`switchSession`, then the shared teardown/apply helpers. Follow the
`createRuntime` factory passed from `packages/coding-agent/src/main.ts`.
...
`buildContextEntries` plus `buildSessionContext`. Use a small hand-drawn tree
with a compaction entry and a branch summary. Trace the active leaf backwards,
...
The terminal user interface (TUI) is the interactive layer that turns session
