...
For a small example, the terminal-facing `coding-agent` package depends on the
generic `agent` package, while `agent` depends on the provider-neutral `ai`
package. The direction tells you where a fix belongs: a terminal component
does not belong in `packages/agent`, and provider JSON does not belong in the
TUI.
...
use the requested or relevant targeted validation. To validate a change
confined to `developer-guide/`, run `quarto render developer-guide` from the
repository root; `npm run check` does not render this book.
...
**Important files** column is relative to `packages/coding-agent/src/`.
...
The table is an orientation map, not an exhaustive API list. To navigate it,
